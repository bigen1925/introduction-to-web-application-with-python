...
1. UTF-8で符号化できないバイナリデータは扱えない（ファイルアップロード時、ファイルの中身は送信しない）
...
# `application/x-www-form-urlencoded` に対応する
...
また、バイナリデータは当面扱いませんので、最初は最も一般的な`application/x-www-form-urlencoded`だけ対応することにしましょう。
