...
![](https://storage.googleapis.com/zenn-user-upload/1blru3z63ejja909xvtf69fw75hu)
...
ちなみに`Body`のところが`b''`と表示されているのは、`空のbytes型`だからです。
リクエストヘッダーを辞書に変換したのと同様、リクエストボディも文字列に変換してしまったほうが扱いやすいのではないかと思う方もいらっしゃると思いますが、`bytes`のままにしているのには事情があります。
