...
socketインスタンスに対して`.accept()`メソッドを呼び出すと、その時点で接続待ちのクライアントがいればその接続を受け付けますが、いない場合は**クライアントから新しく接続要求が来るまでプログラムが停止します。**
...
接続済みのsocketインスタンスに対して`.recv()`メソッドを実行すると、コネクションを通じてクライアントが送ってきたデータを`butes`型で取得します。
